...
function setup() {
    ...
    getNumberInput().addEventListener('input', (e) => {
        // Make sure the input is a valid number and within the specified range
        const numTries = parseInt(e.target.value, 10);
        if (numTries >= 1 && numTries <= 9) {
            tries = numTries; // Update the global tries variable
        }
    })
